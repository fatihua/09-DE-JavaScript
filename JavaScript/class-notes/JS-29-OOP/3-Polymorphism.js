...
class Dergi extends Book{
    ...
    //! overriding
    yilHesapla(){
        return `${this.title} kitabı ${new Date().getFullYear()-this.year} yıllıktır.`;
    }
}
...
//! OVERLOADING
        // Overloading, aynı adla birden fazla fonksiyon tanımlamak demektir. Bu fonksiyonlar aynı adla tanımlanmış olsa da, farklı parametreler alabilirler. Bu sayede, aynı adla birden fazla fonksiyon tanımlayabilir ve bu fonksiyonları farklı parametreler ile çağırabilirsiniz. Bu sayede, fonksiyonların farklı işlevleri olabilir. 
...
function arg(arg1){
 ...
 arg("naber") // naber undefined undefined = çünkü son fonksiyon çalışıyor ve diğer iki parametre undefined dönüyor
 arg("naber","nasılsın","iyimisin") // naber nasılsın iyimisin
 
 //* javascript yukarıdan aşağıya doğru okuduğu için kodları, sadece son fonksiyonu çalıştırıp ona göre işlem yapar. */
