//* ======================================================
//*                   FOREACH METHOD
//! NOT: forEach metodu void bir metottur. (Yani herhangi bir değer döndürmez.log(forEach..)=undefined) terminal işlemdir, diziyi fonksiyon içinde yazdırır ve tüketir bitirir ve siz ekstra birşey istemedikçe diziyi değiştirmez.
...
console.log(prices1); //? orjinal diziyi bozmadı

let toplam = 0

const terminal = prices1.forEach((a)=>console.log(toplam+=a))

console.log(terminal); //undefined. //? forEach işini yapar bitirir gider, değişiklik yapmaz.//
...
// her bir elemanı 2 katıyla çarp ve bütün dizi elemanlarını topla
...
prices3.forEach((p,i,prices)=>{
    toplam3+=p 
    console.log(`${i+1}. toplam ${toplam3}`);

    prices[i] = p*2
    //* bu şekilde diziyi kalıcı olarak değiştirdik
})
console.log(prices3);
...
console.log(yeni);
console.log(names); //! orijinali değiştirmez
...
//Number fazladan bir parantez istiyor, sadece + işareti birşey istemiyor, ikisi de sayıya çeviriyor. toFixed() komutu stringe çeviriyor ilgili olduğu yeri.
...
const ucuzlar = prices2.filter((a)=>a<200)
console.log(ucuzlar);

// prices2.filter((a)=>a<200).forEach((b)=> console.log(b))
// console.log(prices2);

prices2.filter((a)=> a < 200).map((b)=> b * 2).forEach((c)=>console.log(c))
 ...
 //* ornek -n harfiyle biten isimleri tersten yazdırıp yeni bir diziye atın
 ...
 const names2 = [
  ...
  //* ======================================================
//*         PIPELINE (yukarıda da buna örnek yaptık)
//* ======================================================
...
console.log(zengin.concat(fakir)); //concat da split gibi 'string'i 'array'a dönüştürüyor direk
...
// https://dev.to/ra1nbow1/5-ways-to-use-the-reduce-method-in-js-1k8h
//* ======================================================
...
//* örnek - alttaki 1'den 7'ye kadar olan sayıların bulunduğu bir dizi verilmiş ancak bir eleman unutulmuş, unutulan elemanı bulunuz.

const nummern=[5 ,1 ,4 ,7 ,2 ,3 ]

let total = ((nummern.length + 1) * (nummern.length + 2)) / 2

const diziToplam=nummern.reduce((acc, current) => acc + current)
