...
//! 1- Bir Todo Uygulamasında 5 tane kullanıcının todolistinin olduğu Json dosyasında
...
// userTodos.filter((a) => a.userId === 1 && a.completed === true).forEach((a)=>console.log(`${a.title}`))


 //! d - Bu yapıya yeni bir eleman ekleyelim

//  let newTask = {userId:5, id:101, title:"bulaşıkları yıka", completed:true};

//  userTodos.push(newTask) //101
...
// products
//   .filter((a) => a.id === 1) // filter((id)=>id===1) 'de diyebiliriz...
...
//! a - birden fazla ürün ekleme cevabı:
// const addProducts = [
//     {
...
//? süslü içerisinde görmezse (buradaki örnekte olduğu gibi) dot notation ile değil de square brackets ile kodu yazmamız gerek, o zaman curly brackets'ın içindekini görebiliyor.
...
const customerList = [
  ...
  //   İsim - adres ve telefonlarını yeni bir listeye alarak düzenli bir şekilde gösterecektir.
  
  // Beklenen Sonuç:
  // [
  //     {name: 'LEANNE GRAHAM', 
  //    adress: 'Gwenborough city, Kulas Light street Apt. 556 suite',      
  //    phone: '1-770-736-8031 x56442'}
...
//         for (j in i.address){
//             if(i.address[j] != i.address.geo){
//                 newArr1.push({name:i.name, address:i.address[j]})
...
// const newList = customerList.map(({name, address, phone})=>({name: name, address: address.city, address.suite, address.zipcode, address.street, phone:phone}))
// console.log(newList);

// const newList = customerList.map(({name, address, phone})=>({name: name, phone:phone,address:{city:address.city,street:address.street, zipcode:address.zipcode}}))
// console.log(newList);
...
// customerList.filter(({name, phone})=>{
//     let result;
//     if(name.toLocaleLowerCase().startsWith("e")){
//         result = `Pazartesi günü aranacaklar : name : ${name}, phone:${phone}`
//     }else if(name.toLocaleLowerCase().startsWith("c")){
//         result = `Çarşamba günü aranacaklar : name : ${name}, phone:${phone}`
    
//     }else{   // yazılmazsa undefined döndürüyor bir satırı...
//         return;
//     }
//     console.log(result)
//     }
// )
  ...
  // Beklenen Sonuc
  // {name: 'Leanne Graham', email: 'Sincere@april.biz', mesaj: 'Yeni çıkan XXX elektronik cihazımızı mutlaka denemelisiniz '}
  // {name: 'Ervin Howell', email: 'Shanna@melissa.tv', mesaj: ' Yeni Çıkan kahve yapma makinamızı deneyin.Memnun kalacaksınız'}
  

  customerList.map(({name,id,email})=>{
    let result;
    if(id%2){
        result = `${name}, ${email}, mesaj : ' Yeni Çıkan kahve yapma makinamızı deneyin.Memnun kalacaksınız'`
    }else{
        result = `${name}, ${email}, mesaj : 'Yeni çıkan XXX elektronik cihazımızı mutlaka denemelisiniz '`
    //  return result // yazarsak hata veriyor, sadece ilk if'i gösteriyor
    }
    console.log(result);
  }
)
